...
savePaidSale=function(o,method){_oldSavePaidSale(o,method); if(o&&o.table)setTableAuto(o.table,'available'); queueSave();};
...
let salesReportRange = salesReportRange || 'today';
function setSalesReportRange(r){ salesReportRange=r; render(); }
function reportRangeLabel(){return ({today:'1 day(s)',yesterday:'Yesterday',last7:'Last 7 days',month:'This month',lastmonth:'Last month',custom:'Custom'}[salesReportRange]||'1 day(s)');}
...
function saleInRange(s){
  ...
  if(salesReportRange==='last7'){ const start=new Date(now); start.setDate(start.getDate()-6); return d>=new Date(start.getFullYear(),start.getMonth(),start.getDate()) && d<=new Date(now.getFullYear(),now.getMonth(),now.getDate(),23,59,59); }
